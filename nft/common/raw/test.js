...
async function test() {
  ...
  let internal = 1000
  let freq = 30
  let image = await WomboDream.generateImage(style, prompt, token.idToken, null, null, null, null, null, internal, freq)
  let imgUrl = image.result.final
  console.log(image.result)
  // return imgUrl
  if (imgUrl) {
    download(imgUrl, `raw/img/${style}_${prompt}_${internal}_${freq}`)
  } else {
    console.log('error!')
    // test()
  }
...
// test2('QmWXQtmRAh7rkXqB9wkveLho7Kgdrg9uPbfmmpwwHQUzaT')
...
async function test3() {
  ...
  ipfs.add(Buffer.from(JSON.stringify(json)))
    .then(res => {
      console.log(data)
      metaCid = res[0]?.hash
      return ipfs.cat(metaCid)
    })
    .then(output => {
      console.log('data : ', JSON.parse(output))
    })
    return metaCid
}
...
const types = [
  'Paint', 'HDR', 'Polygon', 'gouache', 'Comic', 'Line-Art'
]

async function flowCheck1 (type, prompt, name, owner) {
  // 이미지 생성 후 업로드
  api.imgUpload(type, prompt)
    .then((imgCid) => {
      ...
      api.metaUpload(imgCid, name, owner, type)
        .then((metaData) => {
          // 백에 메타데이터 cid 전달? string data 전달?
          
        })
    })
  // let imgCid = 'QmYsfXFp1CQeTxffASsRKSM2y1uZnWWcxix3whfVN5BYXx'
  // let metaCid = await api.metaUpload(imgCid, name, owner, type)
  // const metaCid = 'QmWFsM7gBWxBsn8WYH4q7ngYgQqqEJ425BCeC8GzoBTpDz'
  // const metaData = await api.getMetaInfo(metaCid)
}

flowCheck1(types[0], 'whale shark', 'enimal', 'wk')
// api.metaUpload()
